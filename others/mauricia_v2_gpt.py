...
MAX_CONTEXT_CHARS = 25000  # recorta contexto para no ahogar a phi3
K_NORMAL = 50              # cantidad de chunks para preguntas normales
K_DINERO = 100            # fuerza bruta cuando preguntan por costos

print("⚙️  Configurando Agente Inteligente USACH...")

# =========================
# 1) LLM (OpenAI / GitHub Models)
# =========================
try:
    llm = ChatOpenAI(
        ...
        temperature=0,
        max_tokens=500  # Un poco más de espacio
    )
    print(f"✓ Cerebro cargado: GPT-4o (vía GitHub)")
...
def es_saludo_puro(user_input: str) -> bool:
    """
    Saludo puro: el mensaje ES un saludo (no contiene preguntas).
    Evita que 'hola cuánto cuesta...' se vaya por el camino de saludo.
    """
    t = (user_input or "").strip().lower()
    t = t.strip("!¡.,;: ")
    return t in SALUDOS_EXACTOS
